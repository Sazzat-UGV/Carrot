                    ...
                    <tbody>
                        @foreach ($cart_content as $row)
                            <tr>
                                ...
                                <td class="col-2 text-center">{{ $setting->currency }}{{ $row->single_price }}</td>
                                <td class="col-1 text-center">{{ $row->qty }}</td>
                                <td class="col-2 text-end">
                                    {{ $setting->currency }}{{ $row->single_price * $row->qty }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="table-responsive">
                <table class="table border border-top-0 mb-0">
                    <tr class="bg-light">
                        ...
                        <td class="col-sm-2 text-end">{{ $setting->currency }}{{ $order->subtotal }}</td>
                    </tr>
                    @if ($order->coupon_code)
                        <tr class="bg-light">
                            ...
                            <td class="col-sm-2 text-end">
                                {{ $setting->currency }}{{ $order->coupon_discount }}</td>
                        </tr>
                    @endif
                    <tr class="bg-light">
                        <td class="text-end"><strong>Tax:(5%)</strong></td>
                        <td class="col-sm-2 text-end">{{ $setting->currency }}{{ $order->tax }}</td>
                    </tr>
                    @if ($order->coupon_code)
                        <tr class="bg-light">
                            <td class="text-end"><strong>Total:</strong></td>
                            <td class="col-sm-2 text-end">
                                {{ $setting->currency }}{{ $order->after_discount + $order->tax }}
                            </td>
                        </tr>
                    @else
                        <tr class="bg-light">
                            <td class="text-end"><strong>Total:</strong></td>
                            <td class="col-sm-2 text-end">
                                {{ $setting->currency }}{{ $order->total }}</td>
                        </tr>
                    @endif
