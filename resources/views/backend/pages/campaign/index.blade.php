                        ...
                        <table id="example" class="table table-" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($campaigns as $index => $campaign)
                                    <tr>
                                        <th>{{ $index + 1 }}</th>
                                        <td><img src="{{ asset('uploads/campaign') }}/{{ $campaign->image }}"
                                                alt="" style="max-width: 100px;"></td>
                                        <td class="wrap">{{ $campaign->title }}</td>
                                        <td class="wrap">{{ $campaign->start_date }}</td>
                                        <td class="wrap">{{ $campaign->end_date }}</td>
                                        <td class="wrap">{{ $campaign->discount }}%</td>
                                        <td>
                                            <label class="switch switch-success">
                                                <input class="switch-input status" type="checkbox"
                                                    data-id="{{ $campaign->id }}" id="campaign-{{ $campaign->id }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#example').on('change', '.status', function() {
                ...
                var url = '{{ route('admin.campaign.status', ':id') }}'.replace(':id', item_id);
